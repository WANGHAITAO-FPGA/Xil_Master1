...
object regFileGen {
  ...
  def genRegFileByMarkdown(doc_name : String)={
    val regFileHdl = new PrintWriter(new File(s"$doc_name.md"))
      ...
      for ((address, tasks) <- busFactory.elementsPerAddress.toList.sortBy(_._1.lowerBound)) {
        for (task <- tasks) task match {
          ...
          case task: BusSlaveFactoryWrite =>{
            if(moduleFirst){
              regContent++=f"$moduleName|0x$baseAddr%x|0x${address.lowerBound}%x|${task.that.getName("not named")}|[${task.bitOffset + widthOf(task.that) - 1}:${task.bitOffset}]|W|${if(task.documentation != null) s" ${task.documentation}" else "no description"}|\n"
              moduleFirst=false
            }else{
