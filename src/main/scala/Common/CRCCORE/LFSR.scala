...
class lfsr_test extends Component{
  val io = new Bundle{
    val lsfr = out Bits(8 bits)
  }
  noIoPrefix()

  val seed  = Reg(Bits(8 bits)) init(100)
  seed := LFSR.Fibonacci(seed, Seq(8,6,5,4))
  io.lsfr := seed
}
...
object lfsr_test{
  ...
  def main(args: Array[String]): Unit = {
    SimConfig.withWave.doSim(new lfsr_test){dut=>
