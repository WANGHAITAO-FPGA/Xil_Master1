...
case class SDAC_REG(addrwidth : Int = 10, endcoder_num : Int = 4, bissc_num : Int = 4, ad7606_num : Int = 3, ad5544_num : Int = 3) extends Component{
  ...
  val Opt1_Single = My_Reg_Opt_Single.fieldAt(1,bc = 1 bits,RO,0,"外部光耦1输入信号，FPGA_DI1")
  ...
  val Opt3_Single = My_Reg_Opt_Single.fieldAt(3,bc = 1 bits,RO,0,"外部光耦3输入信号，FPGA_DI3")
  Opt3_Single := io.FPGA_DI_Filter(2).asBits
  val Opt4_Single = My_Reg_Opt_Single.fieldAt(4,bc = 1 bits,RO,0,"外部光耦4输入信号，FPGA_DI4")
  Opt4_Single := io.FPGA_DI_Filter(3).asBits
  val Opt5_Single = My_Reg_Opt_Single.fieldAt(5,bc = 1 bits,RO,0,"外部光耦5输入信号，FPGA_DI5")
  Opt5_Single := io.FPGA_DI_Filter(4).asBits
  val Opt6_Single = My_Reg_Opt_Single.fieldAt(6,bc = 1 bits,RO,0,"外部光耦6输入信号，FPGA_DI6")
  Opt6_Single := io.FPGA_DI_Filter(5).asBits
  val Opt7_Single = My_Reg_Opt_Single.fieldAt(7,bc = 1 bits,RO,0,"外部光耦7输入信号，FPGA_DI7")
  Opt7_Single := io.FPGA_DI_Filter(6).asBits
  val Opt8_Single = My_Reg_Opt_Single.fieldAt(8,bc = 1 bits,RO,0,"外部光耦8输入信号，FPGA_DI8")
  Opt8_Single := io.FPGA_DI_Filter(7).asBits
  val Opt9_Single = My_Reg_Opt_Single.fieldAt(9,bc = 1 bits,RO,0,"外部光耦9输入信号，FPGA_DI9")
  Opt9_Single := io.FPGA_DI_Filter(8).asBits
  val Opt10_Single = My_Reg_Opt_Single.fieldAt(10,bc = 1 bits,RO,0,"外部光耦10输入信号，FPGA_DI10")
  Opt10_Single := io.FPGA_DI_Filter(9).asBits
  val Opt11_Single = My_Reg_Opt_Single.fieldAt(11,bc = 1 bits,RO,0,"外部光耦11输入信号，FPGA_DI11")
  Opt11_Single := io.FPGA_DI_Filter(10).asBits
  val Opt12_Single = My_Reg_Opt_Single.fieldAt(12,bc = 1 bits,RO,0,"外部光耦12输入信号，FPGA_DI12")
  Opt12_Single := io.FPGA_DI_Filter(11).asBits
  val Opt13_Single = My_Reg_Opt_Single.fieldAt(13,bc = 1 bits,RO,0,"外部光耦13输入信号，FPGA_DI13")
  Opt13_Single := io.FPGA_DI_Filter(12).asBits
  val Opt14_Single = My_Reg_Opt_Single.fieldAt(14,bc = 1 bits,RO,0,"外部光耦14输入信号，FPGA_DI14")
  Opt14_Single := io.FPGA_DI_Filter(13).asBits
  val Opt15_Single = My_Reg_Opt_Single.fieldAt(15,bc = 1 bits,RO,0,"外部光耦15输入信号，FPGA_DI15")
  Opt15_Single := io.FPGA_DI_Filter(14).asBits
  val Opt16_Single = My_Reg_Opt_Single.fieldAt(16,bc = 1 bits,RO,0,"外部光耦16输入信号，FPGA_DI16")
  ...
  val TTL1_Loop = My_Reg_EN_TTL_LOOP.fieldAt(2,bc = 1 bits,RO,0,"外部TTL1输出回环信号")
  TTL1_Loop := io.M_EN_TTL(0).asBits
  val TTL2_Loop = My_Reg_EN_TTL_LOOP.fieldAt(6,bc = 1 bits,RO,0,"外部TTL2输出回环信号")
  TTL2_Loop := io.M_EN_TTL(1).asBits
  val TTL3_Loop = My_Reg_EN_TTL_LOOP.fieldAt(10,bc = 1 bits,RO,0,"外部TTL3输出回环信号")
  TTL3_Loop := io.M_EN_TTL(2).asBits
  val TTL4_Loop = My_Reg_EN_TTL_LOOP.fieldAt(14,bc = 1 bits,RO,0,"外部TTL4输出回环信号")
  TTL4_Loop := io.M_EN_TTL(3).asBits
  val TTL5_Loop = My_Reg_EN_TTL_LOOP.fieldAt(18,bc = 1 bits,RO,0,"外部TTL5输出回环信号")
  TTL5_Loop := io.M_EN_TTL(4).asBits
  val TTL6_Loop = My_Reg_EN_TTL_LOOP.fieldAt(22,bc = 1 bits,RO,0,"外部TTL6输出回环信号")
  TTL6_Loop := io.M_EN_TTL(5).asBits

  val My_Reg_EN_TTL_LOOP1 = busslave.newReg("2路外部输出TTL信号回环信号，M_EN_TTL_LOOP1")
  val TTL7_Loop = My_Reg_EN_TTL_LOOP1.fieldAt(18,bc = 1 bits,RO,0,"外部TTL7输出回环信号")
  TTL7_Loop := io.M_EN_TTL(6).asBits
  val TTL8_Loop = My_Reg_EN_TTL_LOOP1.fieldAt(26,bc = 1 bits,RO,0,"外部TTL8输出回环信号")
  ...
  val Opt2_Loop = My_Reg_Opt_Loop.fieldAt(2,bc = 1 bits,RO,0,"外部光耦2输出回环信号")
  Opt2_Loop := io.FPGA_DO(1).asBits
  val Opt3_Loop = My_Reg_Opt_Loop.fieldAt(4,bc = 1 bits,RO,0,"外部光耦3输出回环信号")
  Opt3_Loop := io.FPGA_DO(2).asBits
  val Opt4_Loop = My_Reg_Opt_Loop.fieldAt(6,bc = 1 bits,RO,0,"外部光耦4输出回环信号")
  Opt4_Loop := io.FPGA_DO(3).asBits
  val Opt5_Loop = My_Reg_Opt_Loop.fieldAt(8,bc = 1 bits,RO,0,"外部光耦5输出回环信号")
  Opt5_Loop := io.FPGA_DO(4).asBits
  val Opt6_Loop = My_Reg_Opt_Loop.fieldAt(10,bc = 1 bits,RO,0,"外部光耦6输出回环信号")
  Opt6_Loop := io.FPGA_DO(5).asBits
  val Opt7_Loop = My_Reg_Opt_Loop.fieldAt(12,bc = 1 bits,RO,0,"外部光耦7输出回环信号")
  Opt7_Loop := io.FPGA_DO(6).asBits
  val Opt8_Loop = My_Reg_Opt_Loop.fieldAt(14,bc = 1 bits,RO,0,"外部光耦8输出回环信号")
  Opt8_Loop := io.FPGA_DO(7).asBits
  val Opt9_Loop = My_Reg_Opt_Loop.fieldAt(16,bc = 1 bits,RO,0,"外部光耦9输出回环信号")
  Opt9_Loop := io.FPGA_DO(8).asBits
  val Opt10_Loop = My_Reg_Opt_Loop.fieldAt(18,bc = 1 bits,RO,0,"外部光耦10输出回环信号")
  Opt10_Loop := io.FPGA_DO(9).asBits
  val Opt11_Loop = My_Reg_Opt_Loop.fieldAt(20,bc = 1 bits,RO,0,"外部光耦11输出回环信号")
  Opt11_Loop := io.FPGA_DO(10).asBits
  val Opt12_Loop = My_Reg_Opt_Loop.fieldAt(22,bc = 1 bits,RO,0,"外部光耦12输出回环信号")
  Opt12_Loop := io.FPGA_DO(11).asBits
  val Opt13_Loop = My_Reg_Opt_Loop.fieldAt(24,bc = 1 bits,RO,0,"外部光耦13输出回环信号")
  Opt13_Loop := io.FPGA_DO(12).asBits
  val Opt14_Loop = My_Reg_Opt_Loop.fieldAt(26,bc = 1 bits,RO,0,"外部光耦14输出回环信号")
  Opt14_Loop := io.FPGA_DO(13).asBits
  val Opt15_Loop = My_Reg_Opt_Loop.fieldAt(28,bc = 1 bits,RO,0,"外部光耦15输出回环信号")
  Opt15_Loop := io.FPGA_DO(14).asBits
  val Opt16_Loop = My_Reg_Opt_Loop.fieldAt(30,bc = 1 bits,RO,0,"外部光耦16输出回环信号")
  ...
  val M_EN_TTL1 = My_Reg_M_EN_TTL.fieldAt(2,bc = 1 bits,RW,0,"外部TTL1输出信号，M_EN_TTL1")
  io.M_EN_TTL(0) := M_EN_TTL1.asBool
  val M_EN_TTL2 = My_Reg_M_EN_TTL.fieldAt(6,bc = 1 bits,RW,0,"外部TTL2输出信号，M_EN_TTL2")
  io.M_EN_TTL(1) := M_EN_TTL2.asBool
  val M_EN_TTL3 = My_Reg_M_EN_TTL.fieldAt(10,bc = 1 bits,RW,0,"外部TTL3输出信号，M_EN_TTL3")
  io.M_EN_TTL(2) := M_EN_TTL3.asBool
  val M_EN_TTL4 = My_Reg_M_EN_TTL.fieldAt(14,bc = 1 bits,RW,0,"外部TTL4输出信号，M_EN_TTL4")
  io.M_EN_TTL(3) := M_EN_TTL4.asBool
  val M_EN_TTL5 = My_Reg_M_EN_TTL.fieldAt(18,bc = 1 bits,RW,0,"外部TTL5输出信号，M_EN_TTL5")
  io.M_EN_TTL(4) := M_EN_TTL5.asBool
  val M_EN_TTL6 = My_Reg_M_EN_TTL.fieldAt(22,bc = 1 bits,RW,0,"外部TTL6输出信号，M_EN_TTL6")
  io.M_EN_TTL(5) := M_EN_TTL6.asBool

  val My_Reg_M_EN_TTL1 = busslave.newReg(doc="2路外部输出TTL信号（例如：PA 使能），M_EN_TTL1")
  val M_EN_TTL7 = My_Reg_M_EN_TTL1.fieldAt(18,bc = 1 bits,RW,0,"外部TTL7输出信号，M_EN_TTL7")
  io.M_EN_TTL(6) := M_EN_TTL7.asBool
  val M_EN_TTL8 = My_Reg_M_EN_TTL1.fieldAt(26,bc = 1 bits,RW,0,"外部TTL8输出信号，M_EN_TTL8")
