...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.srio_clk );
      signalAccess[1] = new CDataSignalAccess( top.reset );
      signalAccess[2] = new CDataSignalAccess( top.iresp_stream_valid );
      signalAccess[3] = new CDataSignalAccess( top.iresp_stream_ready );
      signalAccess[4] = new QDataSignalAccess( top.iresp_stream_payload );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\SpinalWorkSpace\\Xil_Master1\\.\\simWorkspace\\srio_tt/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\SpinalWorkSpace\\Xil_Master1\\.\\simWorkspace\\srio_tt/") + name + ".dat").c_str());
      #endif
    }
